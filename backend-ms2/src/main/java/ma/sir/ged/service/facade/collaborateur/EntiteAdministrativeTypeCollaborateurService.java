package ma.sir.ged.service.facade.collaborateur;

import java.util.List;
import ma.sir.ged.bean.core.EntiteAdministrativeType;
import ma.sir.ged.dao.criteria.core.EntiteAdministrativeTypeCriteria;
import ma.sir.ged.dao.criteria.history.EntiteAdministrativeTypeHistoryCriteria;
import ma.sir.ged.zynerator.service.IService;


public interface EntiteAdministrativeTypeCollaborateurService extends  IService<EntiteAdministrativeType,EntiteAdministrativeTypeCriteria, EntiteAdministrativeTypeHistoryCriteria>  {




}
